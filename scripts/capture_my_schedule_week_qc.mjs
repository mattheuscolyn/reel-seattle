/**
 * Capture My Schedule Week Stage 1 QC screenshots.
 * Run: node scripts/capture_my_schedule_week_qc.mjs
...
const BASE = 'http://127.0.0.1:5175/?myScheduleWeek=1';
...
async function openScheduleWeek(page) {
  await page.goto('http://127.0.0.1:5175/', { waitUntil: 'networkidle' });
  await clearLocal(page);
  await page.goto(BASE, { waitUntil: 'networkidle' });
  await page.waitForSelector('[data-schedule-source="mockup-fixture"]', {
    timeout: 15_000,
  });
}
...
try {
  for (const vp of WIDTHS) {
    ...
    if (shellOverflow) {
      throw new Error(`Page-level horizontal overflow at ${vp.name}`);
    }
    ...
    if (backCount !== 0) {
      throw new Error(`Unexpected back button at ${vp.name}`);
    }

    await page.screenshot({
      path: join(OUT, `i-schedule-week-${vp.name}-default.png`),
      fullPage: true,
    });

    await page
      .locator('[data-schedule-plan-group="group-sat-multi"] .v2-msw-plan-group-hint')
      .click();
    await page.screenshot({
      path: join(OUT, `i-schedule-week-${vp.name}-grouped.png`),
      fullPage: true,
    });

    await page.getByLabel('Next week').click();
    await page.waitForSelector('[data-schedule-week="week-jul-26-aug-1"]', {
      timeout: 10_000,
    });
    await page.screenshot({
      path: join(OUT, `i-schedule-week-${vp.name}-alt-week.png`),
      fullPage: true,
    });
  ...
  await openScheduleWeek(scrollPage);
  await scrollPage.evaluate(() => {
    const el = document.querySelector('.v2-msw-timeline-scroll');
    if (el) el.scrollLeft = 120;
  });
  await scrollPage.screenshot({
    path: join(OUT, 'i-schedule-week-393-timeline-scroll.png'),
    fullPage: true,
  });
