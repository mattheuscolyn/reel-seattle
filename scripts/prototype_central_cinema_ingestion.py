#!/usr/bin/env python3
"""Central Cinema independent-source observation prototype (non-production).
...
from reel_seattle.prototypes.central_cinema import (  # noqa: E402
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Prototype Central Cinema contract ingestion (non-production)."
    )
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("local-output/central-cinema-prototype"),
    )
    parser.add_argument("--fixture-dir", type=Path, default=None, help="Offline HTML fixture directory")
    parser.add_argument("--live", action="store_true", help="Fetch live Central Cinema pages (read-only)")
...
def _load_fixture_pages(fixture_dir: Path) -> dict[str, str]:
    if not fixture_dir.is_dir():
        raise CentralCinemaPrototypeError(f"fixture dir not found: {fixture_dir}")
    pages: dict[str, str] = {}
    manifest = fixture_dir / "manifest.json"
    if manifest.is_file():
        mapping = json.loads(manifest.read_text(encoding="utf-8"))
        if not isinstance(mapping, dict):
            raise CentralCinemaPrototypeError("manifest.json must be an object of url->filename")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.live and args.fixture_dir:
            raise CentralCinemaPrototypeError("use either --live or --fixture-dir, not both")
        if not args.live and not args.fixture_dir:
            raise CentralCinemaPrototypeError("provide --fixture-dir or --live")
        ...
        result_path = out_dir / "central_cinema_independent_source_result.json"
        summary_path = out_dir / "central_cinema_prototype_summary.json"
        result_path.write_text(serialize_independent_source_result(result), encoding="utf-8")
        summary = summarize_result(result)
        summary_path.write_text(json.dumps(summary, indent=2) + "\n", encoding="utf-8")

        print("Central Cinema prototype complete (non-production)")
        for key, value in summary.items():
            print(f"  {key}: {value}")
        print(f"  wrote: {result_path}")
        print(f"  wrote: {summary_path}")
        return 0
    except (OSError, json.JSONDecodeError, CentralCinemaPrototypeError, ValueError) as exc:
