...
_VARIANT_SUFFIX_PATTERNS: tuple[re.Pattern[str], ...] = tuple(
    ...
    for pattern in (
        ...
        r"\s+sensory\s+friendly(?:\s+screening)?",
        r":\s*community\s+screening",
        r"\s+community\s+screening",
        r":\s*sing[- ]?along(?:\s+screening)?",
        r"\s+sing[- ]?along(?:\s+screening)?",
        r":\s*early\s+access(?:\s+screening)?(?:\s+with\s+.+)?",
        r"\s+early\s+access(?:\s+screening)?(?:\s+with\s+.+)?",
        r":\s*fan\s+event",
        r"\s+fan\s+event",
        r":\s*fan\s+first(?:\s+screening)?",
        r"\s+fan\s+first(?:\s+screening)?",
        r":\s*opening\s+night(?:\s+(?:event|fan\s+event))?",
        r"\s+opening\s+night(?:\s+(?:event|fan\s+event))?",
        r":\s*imax\s+opening\s+night(?:\s+fan\s+event)?",
        r"\s+imax\s+opening\s+night(?:\s+fan\s+event)?",
        r":\s*sneak\s+(?:peek|preview).*",
        r"\s+sneak\s+(?:peek|preview).*",
        r"\s*[-–—:]\s*special\s+screening(?:\s*\+\s*q\s*&\s*a)?.*",
        r"\s*\+\s*q\s*&\s*a(?:\s+with\s+.+)?",
        r"\s*[-–—:]\s*q\s*&\s*a(?:\s+with\s+.+)?",
        r"\s+q\s*&\s*a(?:\s+with\s+.+)?",
        r"\s*[-–—:]\s*q\s*and\s*a(?:\s+with\s+.+)?",
        r"\s+live\s+q\s*&\s*a(?:\s+with\s+.+)?",
        r"\s+with\s+(?:the\s+)?(?:director|filmmaker|cast(?:\s+member)?|guest|actor).+",
        r":\s*screen\s+unseen.*",
        r"\s+screen\s+unseen.*",
        r":\s*scream\s+unseen.*",
        r"\s+scream\s+unseen.*",
        ...
        r"\s+double\s+feature",
        r":\s*\d+(?:st|nd|rd|th)\s+anniversary(?:\s+(?:double\s+feature|screening|event))?",
        r"\s+\d+(?:st|nd|rd|th)\s+anniversary(?:\s+(?:double\s+feature|screening|event))?",
        r":\s*anniversary(?:\s+(?:screening|event|double\s+feature))",
        r"\s+anniversary(?:\s+(?:screening|event|double\s+feature))",
        r":\s*encore(?:\s+screening)?",
        r"\s+encore(?:\s+screening)?",
        r":\s*live(?:\s+in\s+concert)?",
        r"\s+live(?:\s+in\s+concert)?",
        r":\s*closed\s+caption(?:ed|ing)?",
        r"\s+closed\s+caption(?:ed|ing)?",
        r":\s*audio\s+description",
        r"\s+audio\s+description",
        r"\s+\(\s*imax\s*\)",
        r"\s+\(\s*3d\s*\)",
