...
* **SIFF:** any failed discovered film/program page → `partial_failure`, `restate_safe=false` (source-wide; parsed rows remain in the log for inspection but do not replace history). Zero discovered programs without affirmative empty-listing proof is structural failure (P-20B). Occurrence-level venue/date/identity rejects also block restatement.
